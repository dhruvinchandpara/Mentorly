...
export default function ManageAdmins() {
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      <div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Manage Admins</h1>
        <p className="text-primary text-sm mt-1">Grant admin access and view existing administrators.</p>
      </div>
      ...
      <Card className="border-border shadow-sm">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base text-foreground">Grant Admin Access</CardTitle>
          </div>
          <CardDescription>
        ...
        <CardContent>
          ...
          <div className="mt-4 p-4 bg-warning-bg rounded-lg border border-warning">
            <h4 className="text-xs font-bold text-warning uppercase tracking-wider mb-1">Important Note</h4>
      ...
      <Card className="border-border shadow-sm">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-base text-foreground">Current Administrators</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <p className="text-sm text-[var(--fg-faint)] py-8 text-center">No administrators found.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="bg-accent/50 hover:bg-accent/50">
                  ...
                  <TableHead className="text-primary font-semibold text-right">Role</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {admins.map((admin) => (
                  <TableRow key={admin.id} className="hover:bg-accent/30">
                    <TableCell className="font-medium text-foreground">{admin.full_name || 'Unknown'}</TableCell>
                    <TableCell className="text-primary">{admin.email}</TableCell>
                    <TableCell className="text-right">
                      <Badge className="bg-accent text-primary border-border">
