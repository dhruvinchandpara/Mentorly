...
export function RejectReasonModal({ open, onOpenChange, onConfirm, submitting = false, error = null }: RejectReasonModalProps) {
  ...
  // that came from a user-driven close (onOpenChange) or a controlled `open` prop
  // change from the parent (e.g. closing on a successful reject). Base UI's Dialog
  // does not invoke onOpenChange for the latter, so this must be driven by `open`.
  useEffect(() => {
    if (!open) setReason('');
  }, [open]);
